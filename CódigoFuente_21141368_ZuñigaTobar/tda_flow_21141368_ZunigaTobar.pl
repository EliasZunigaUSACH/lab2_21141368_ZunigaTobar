...
:- module(tda_flow_21141368_ZunigaTobar, [getFlowId/2, getFlowsIds/3, agregarOptions/3, flow/4, flowAddOption/3]).
...
agregarOptions([], OptionsAcc, OptionsAcc):- !.
agregarOptions([OptionAgregar|RestoOptions], OptionsAcc, OptionsSalida):-
    getOptionId(OptionAgregar, Id),
    getOptionsIds(OptionsAcc, [], Ids),
    not(member(Id, Ids)),
    agregarOptions(RestoOptions, [OptionAgregar|OptionsAcc], OptionsSalida).
...
flow(Id, NameMessage, Options, [Id, NameMessage, OptionsSalida]):-
    agregarOptions(Options, [], OptionsSalida).
